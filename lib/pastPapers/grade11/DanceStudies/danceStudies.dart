...
class DanceStudiesGrade11Page extends StatefulWidget {
  const DanceStudiesGrade11Page({super.key});

  @override
  _DanceStudiesGrade11PageState createState() =>
      _DanceStudiesGrade11PageState();
}

class _DanceStudiesGrade11PageState extends State<DanceStudiesGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/DanceStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/DanceStudies/$fileName';
