...
class DesignGrade11Page extends StatefulWidget {
  const DesignGrade11Page({super.key});

  @override
  _DesignGrade11PageState createState() => _DesignGrade11PageState();
}

class _DesignGrade11PageState extends State<DesignGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_11/Design');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/Design/$fileName';
