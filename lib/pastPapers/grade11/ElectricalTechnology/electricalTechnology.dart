...
class ElectricalTechnologyGrade11Page extends StatefulWidget {
  const ElectricalTechnologyGrade11Page({super.key});

  @override
  _ElectricalTechnologyGrade11PageState createState() =>
      _ElectricalTechnologyGrade11PageState();
}

class _ElectricalTechnologyGrade11PageState
    extends State<ElectricalTechnologyGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/ElectricalTechnology');
  ...
  void _sortPdfFiles(List<FileObject> objects) {
    ...
    for (final file in pdfFiles) {
      ...
      } else if (file.name.contains('Power')) {
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/ElectricalTechnology/$fileName';
  ...
  Widget build(BuildContext context) {
    final Map<String, List<FileObject>> groupedFiles = {
      'Digital': digitalPdfs,
      'Electronics': electronicsPdfs,
      'Power Systems': powerSystemsPdfs,
    };
