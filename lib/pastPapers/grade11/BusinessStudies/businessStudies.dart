...
class BusinessStudiesGrade11Page extends StatefulWidget {
  const BusinessStudiesGrade11Page({super.key});

  @override
  _BusinessStudiesGrade11PageState createState() =>
      _BusinessStudiesGrade11PageState();
}

class _BusinessStudiesGrade11PageState
    extends State<BusinessStudiesGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/BusinessStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/BusinessStudies/$fileName';
