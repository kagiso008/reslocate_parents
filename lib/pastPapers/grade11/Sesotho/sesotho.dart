...
class SesothoGrade11Page extends StatefulWidget {
  const SesothoGrade11Page({super.key});

  @override
  _SesothoGrade11PageState createState() => _SesothoGrade11PageState();
}

class _SesothoGrade11PageState extends State<SesothoGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_11/Sesotho');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/Sesotho/$fileName';
