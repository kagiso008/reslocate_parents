...
class PhysicalSciencesGrade11Page extends StatefulWidget {
  const PhysicalSciencesGrade11Page({super.key});

  @override
  _PhysicalSciencesGrade11PageState createState() =>
      _PhysicalSciencesGrade11PageState();
}

class _PhysicalSciencesGrade11PageState
    extends State<PhysicalSciencesGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/PhysicalSciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/PhysicalSciences/$fileName';
