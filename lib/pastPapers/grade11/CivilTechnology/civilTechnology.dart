...
class CivilTechnologyGrade11Page extends StatefulWidget {
  const CivilTechnologyGrade11Page({super.key});

  @override
  _CivilTechnologyGrade11PageState createState() =>
      _CivilTechnologyGrade11PageState();
}

class _CivilTechnologyGrade11PageState
    extends State<CivilTechnologyGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/CivilTechnology');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/CivilTechnology/$fileName';
  ...
  Widget build(BuildContext context) {
    final Map<String, List<FileObject>> groupedFiles = {
      'Civil Services': civilServicesFiles,
      'Construction': constructionFiles,
      'Woodworking': woodworkingFiles,
    };
