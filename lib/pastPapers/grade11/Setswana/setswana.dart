...
class SetswanaGrade11Page extends StatefulWidget {
  const SetswanaGrade11Page({super.key});

  @override
  _SetswanaGrade11PageState createState() => _SetswanaGrade11PageState();
}

class _SetswanaGrade11PageState extends State<SetswanaGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_11/Setswana');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/Setswana/$fileName';
