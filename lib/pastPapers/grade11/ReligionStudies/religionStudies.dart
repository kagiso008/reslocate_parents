...
class ReligionStudiesGrade11Page extends StatefulWidget {
  const ReligionStudiesGrade11Page({super.key});

  @override
  _ReligionStudiesGrade11PageState createState() =>
      _ReligionStudiesGrade11PageState();
}

class _ReligionStudiesGrade11PageState
    extends State<ReligionStudiesGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/ReligionStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/ReligionStudies/$fileName';
