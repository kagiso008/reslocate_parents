...
class MathsGrade11Page extends StatefulWidget {
  const MathsGrade11Page({super.key});

  @override
  _MathsGrade11PageState createState() => _MathsGrade11PageState();
}

class _MathsGrade11PageState extends State<MathsGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/Mathematics');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/Mathematics/$fileName';
