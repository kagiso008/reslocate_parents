...
class DramaticArtsGrade11Page extends StatefulWidget {
  const DramaticArtsGrade11Page({super.key});

  @override
  _DramaticArtsGrade11PageState createState() =>
      _DramaticArtsGrade11PageState();
}

class _DramaticArtsGrade11PageState extends State<DramaticArtsGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/DramaticArts');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/DramaticArts/$fileName';
