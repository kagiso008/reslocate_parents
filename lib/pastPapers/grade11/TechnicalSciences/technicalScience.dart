...
class TechnicalScienceGrade11Page extends StatefulWidget {
  const TechnicalScienceGrade11Page({super.key});

  @override
  _TechnicalScienceGrade11PageState createState() =>
      _TechnicalScienceGrade11PageState();
}

class _TechnicalScienceGrade11PageState
    extends State<TechnicalScienceGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/TechnicalSciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/TechnicalSciences/$fileName';
