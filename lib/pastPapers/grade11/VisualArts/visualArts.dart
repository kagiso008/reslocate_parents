...
class VisualArtsGrade11Page extends StatefulWidget {
  const VisualArtsGrade11Page({super.key});

  @override
  _VisualArtsGrade11PageState createState() => _VisualArtsGrade11PageState();
}

class _VisualArtsGrade11PageState extends State<VisualArtsGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_11/VisualArts');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/VisualArts/$fileName';
