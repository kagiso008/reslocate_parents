...
class ConsumerStudiesGrade11Page extends StatefulWidget {
  ...
  @override
  _ConsumerStudiesGrade11PageState createState() =>
      _ConsumerStudiesGrade11PageState();
}

class _ConsumerStudiesGrade11PageState
    extends State<ConsumerStudiesGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/ConsumerStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/ConsumerStudies/$fileName';
