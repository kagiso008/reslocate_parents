...
class HistoryGrade11Page extends StatefulWidget {
  ...
  @override
  _HistoryGrade11PageState createState() => _HistoryGrade11PageState();
}
...
class _HistoryGrade11PageState extends State<HistoryGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_11/History');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/History/$fileName';
