...
class HospitalityStudiesGrade11Page extends StatefulWidget {
  const HospitalityStudiesGrade11Page({super.key});

  @override
  _HospitalityStudiesGrade11PageState createState() =>
      _HospitalityStudiesGrade11PageState();
}

class _HospitalityStudiesGrade11PageState
    extends State<HospitalityStudiesGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/HospitalityStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/HospitalityStudies/$fileName';
