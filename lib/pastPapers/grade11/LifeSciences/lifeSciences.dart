...
class LifeSciencesGrade11Page extends StatefulWidget {
  const LifeSciencesGrade11Page({super.key});

  @override
  _LifeSciencesGrade11PageState createState() =>
      _LifeSciencesGrade11PageState();
}

class _LifeSciencesGrade11PageState extends State<LifeSciencesGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/LifeSciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/LifeSciences/$fileName';
