...
class CATGrade11Page extends StatefulWidget {
  const CATGrade11Page({super.key});

  @override
  _CATGrade11PageState createState() => _CATGrade11PageState();
}

class _CATGrade11PageState extends State<CATGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/ComputerApplicationsTechnology');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/ComputerApplicationsTechnology/$fileName';
