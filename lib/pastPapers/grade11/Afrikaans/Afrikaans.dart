...
class AfrikaansGrade11Page extends StatefulWidget {
  const AfrikaansGrade11Page({super.key});

  @override
  _AfrikaansGrade11PageState createState() => _AfrikaansGrade11PageState();
}

class _AfrikaansGrade11PageState extends State<AfrikaansGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_11/Afrikaans');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/Afrikaans/$fileName';
