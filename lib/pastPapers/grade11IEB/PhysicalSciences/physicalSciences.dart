...
class PhysicalSciencesGrade11IEBPage extends StatefulWidget {
  const PhysicalSciencesGrade11IEBPage({super.key});

  @override
  _PhysicalSciencesGrade11IEBPageState createState() =>
      _PhysicalSciencesGrade11IEBPageState();
}

class _PhysicalSciencesGrade11IEBPageState
    extends State<PhysicalSciencesGrade11IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/IEB/Physical Sciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/IEB/Physical Sciences/$fileName';
  ...
  Widget build(BuildContext context) {
    ...
    for (var file in pdfFiles) {
      ...
      final subject = name.contains('P12')
          ? 'Paper 1'
          : name.contains('P2')
              ? 'Paper 2'
              : 'All Papers';
