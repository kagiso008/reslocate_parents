...
class BusinessStudiesGrade11IEBPage extends StatefulWidget {
  const BusinessStudiesGrade11IEBPage({super.key});

  @override
  _BusinessStudiesGrade11IEBPageState createState() =>
      _BusinessStudiesGrade11IEBPageState();
}

class _BusinessStudiesGrade11IEBPageState
    extends State<BusinessStudiesGrade11IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/IEB/Business Studies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/IEB/Business Studies/$fileName';
