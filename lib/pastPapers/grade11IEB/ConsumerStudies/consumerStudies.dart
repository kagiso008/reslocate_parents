...
class ConsumerStudiesGrade11IEBPage extends StatefulWidget {
  const ConsumerStudiesGrade11IEBPage({super.key});

  @override
  _ConsumerStudiesGrade11IEBPageState createState() =>
      _ConsumerStudiesGrade11IEBPageState();
}

class _ConsumerStudiesGrade11IEBPageState
    extends State<ConsumerStudiesGrade11IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/IEB/Consumer Studies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/IEB/Consumer Studies/$fileName';
