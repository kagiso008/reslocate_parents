...
class GeographyGrade11IEBPage extends StatefulWidget {
  const GeographyGrade11IEBPage({super.key});

  @override
  _GeographyGrade11IEBPageState createState() =>
      _GeographyGrade11IEBPageState();
}

class _GeographyGrade11IEBPageState extends State<GeographyGrade11IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/IEB/Geography');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/IEB/Geography/$fileName';
