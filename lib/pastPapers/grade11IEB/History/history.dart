...
class HistoryGrade11IEBPage extends StatefulWidget {
  const HistoryGrade11IEBPage({super.key});

  @override
  _HistoryGrade11IEBPageState createState() => _HistoryGrade11IEBPageState();
}

class _HistoryGrade11IEBPageState extends State<HistoryGrade11IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/IEB/History');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/IEB/History/$fileName';
