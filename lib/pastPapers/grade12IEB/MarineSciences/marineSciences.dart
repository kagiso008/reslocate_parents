...
class MarineSciencesGrade12IEBPage extends StatefulWidget {
  const MarineSciencesGrade12IEBPage({super.key});

  @override
  _MarineSciencesGrade12IEBPageState createState() =>
      _MarineSciencesGrade12IEBPageState();
}

class _MarineSciencesGrade12IEBPageState
    extends State<MarineSciencesGrade12IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/IEB/MarineSciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/IEB/MarineSciences/$fileName';
