...
class PortugueseSALGrade12IEBPage extends StatefulWidget {
  const PortugueseSALGrade12IEBPage({super.key});

  @override
  _PortugueseSALGrade12IEBPageState createState() =>
      _PortugueseSALGrade12IEBPageState();
}

class _PortugueseSALGrade12IEBPageState
    extends State<PortugueseSALGrade12IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/IEB/PortugueseSAL');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/IEB/PortugueseSAL/$fileName';
