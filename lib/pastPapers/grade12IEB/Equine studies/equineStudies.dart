...
class EquineStudiesGrade12IEBPage extends StatefulWidget {
  const EquineStudiesGrade12IEBPage({super.key});

  @override
  _EquineStudiesGrade12IEBPageState createState() =>
      _EquineStudiesGrade12IEBPageState();
}

class _EquineStudiesGrade12IEBPageState
    extends State<EquineStudiesGrade12IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/IEB/EquineStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/IEB/EquineStudies/$fileName';
