...
class GreekSALGrade12IEBPage extends StatefulWidget {
  const GreekSALGrade12IEBPage({super.key});

  @override
  _GreekSALGrade12IEBPageState createState() => _GreekSALGrade12IEBPageState();
}

class _GreekSALGrade12IEBPageState extends State<GreekSALGrade12IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/IEB/GreekSAL');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/IEB/GreekSAL/$fileName';
