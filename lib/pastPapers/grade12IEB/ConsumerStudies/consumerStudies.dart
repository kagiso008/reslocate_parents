...
class ConsumerStudiesGrade12IEBPage extends StatefulWidget {
  const ConsumerStudiesGrade12IEBPage({super.key});

  @override
  _ConsumerStudiesGrade12IEBPageState createState() =>
      _ConsumerStudiesGrade12IEBPageState();
}

class _ConsumerStudiesGrade12IEBPageState
    extends State<ConsumerStudiesGrade12IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/IEB/ConsumerStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/IEB/ConsumerStudies/$fileName';
