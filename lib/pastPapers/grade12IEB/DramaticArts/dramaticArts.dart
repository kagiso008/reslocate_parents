...
class DramaticArtsGrade12IEBPage extends StatefulWidget {
  const DramaticArtsGrade12IEBPage({super.key});

  @override
  _DramaticArtsGrade12IEBPageState createState() =>
      _DramaticArtsGrade12IEBPageState();
}

class _DramaticArtsGrade12IEBPageState
    extends State<DramaticArtsGrade12IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/IEB/DramaticArts');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/IEB/DramaticArts/$fileName';
