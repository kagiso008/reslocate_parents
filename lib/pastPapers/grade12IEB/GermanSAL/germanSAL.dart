...
class GermanSALGrade12IEBPage extends StatefulWidget {
  const GermanSALGrade12IEBPage({super.key});

  @override
  _GermanSALGrade12IEBPageState createState() =>
      _GermanSALGrade12IEBPageState();
}

class _GermanSALGrade12IEBPageState extends State<GermanSALGrade12IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/IEB/GermanSAL');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/IEB/GermanSAL/$fileName';
