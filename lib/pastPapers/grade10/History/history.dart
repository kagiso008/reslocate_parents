...
class HistoryGrade10Page extends StatefulWidget {
  ...
  @override
  _HistoryGrade10PageState createState() => _HistoryGrade10PageState();
}
...
class _HistoryGrade10PageState extends State<HistoryGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_10/HISTORY');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/HISTORY/$fileName';
