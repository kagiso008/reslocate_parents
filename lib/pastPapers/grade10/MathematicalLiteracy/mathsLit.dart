...
class MathematicalLiteracyGrade10Page extends StatefulWidget {
  const MathematicalLiteracyGrade10Page({super.key});

  @override
  _MathematicalLiteracyGrade10PageState createState() =>
      _MathematicalLiteracyGrade10PageState();
}

class _MathematicalLiteracyGrade10PageState
    extends State<MathematicalLiteracyGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/MATHEMATICAL LITERACY');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/MATHEMATICAL LITERACY/$fileName';
