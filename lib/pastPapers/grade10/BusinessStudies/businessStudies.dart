...
class BusinessStudiesGrade10Page extends StatefulWidget {
  const BusinessStudiesGrade10Page({super.key});

  @override
  _BusinessStudiesGrade10PageState createState() =>
      _BusinessStudiesGrade10PageState();
}

class _BusinessStudiesGrade10PageState
    extends State<BusinessStudiesGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/BUSINESS STUDIES');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/BUSINESS STUDIES/$fileName';
