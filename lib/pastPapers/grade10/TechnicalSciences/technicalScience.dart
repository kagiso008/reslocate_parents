...
class TechnicalScienceGrade10Page extends StatefulWidget {
  const TechnicalScienceGrade10Page({super.key});

  @override
  _TechnicalScienceGrade10PageState createState() =>
      _TechnicalScienceGrade10PageState();
}

class _TechnicalScienceGrade10PageState
    extends State<TechnicalScienceGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/TECHNICAL SCIENCES');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/TECHNICAL SCIENCES/$fileName';
