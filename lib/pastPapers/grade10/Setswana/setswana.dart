...
class SetswanaGrade10Page extends StatefulWidget {
  const SetswanaGrade10Page({super.key});

  @override
  _SetswanaGrade10PageState createState() => _SetswanaGrade10PageState();
}

class _SetswanaGrade10PageState extends State<SetswanaGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_10/SETSWANA');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/SETSWANA/$fileName';
