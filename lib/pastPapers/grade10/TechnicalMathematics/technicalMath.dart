...
class TechnicalMathsGrade10Page extends StatefulWidget {
  const TechnicalMathsGrade10Page({super.key});

  @override
  _TechnicalMathsGrade10PageState createState() =>
      _TechnicalMathsGrade10PageState();
}

class _TechnicalMathsGrade10PageState extends State<TechnicalMathsGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/TECHNICAL MATHEMATICS');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/TECHNICAL MATHEMATICS/$fileName';
