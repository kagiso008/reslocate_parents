...
class EconomicsGrade10Page extends StatefulWidget {
  const EconomicsGrade10Page({super.key});

  @override
  _EconomicsGrade10PageState createState() => _EconomicsGrade10PageState();
}

class _EconomicsGrade10PageState extends State<EconomicsGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_10/ECONOMICS');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/ECONOMICS/$fileName';
