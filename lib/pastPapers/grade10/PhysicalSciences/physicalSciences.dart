...
class PhysicalSciencesGrade10Page extends StatefulWidget {
  const PhysicalSciencesGrade10Page({super.key});

  @override
  _PhysicalSciencesGrade10PageState createState() =>
      _PhysicalSciencesGrade10PageState();
}

class _PhysicalSciencesGrade10PageState
    extends State<PhysicalSciencesGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/PHYSICAL SCIENCES');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/PHYSICAL SCIENCES/$fileName';
