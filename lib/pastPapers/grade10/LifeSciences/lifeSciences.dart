...
class LifeSciencesGrade10Page extends StatefulWidget {
  ...
  @override
  _LifeSciencesGrade10PageState createState() =>
      _LifeSciencesGrade10PageState();
}

class _LifeSciencesGrade10PageState extends State<LifeSciencesGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/LIFE SCIENCES');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/LIFE SCIENCES/$fileName';
