...
class BusinessStudiesGrade10IEBPage extends StatefulWidget {
  const BusinessStudiesGrade10IEBPage({super.key});

  @override
  _BusinessStudiesGrade10IEBPageState createState() =>
      _BusinessStudiesGrade10IEBPageState();
}

class _BusinessStudiesGrade10IEBPageState
    extends State<BusinessStudiesGrade10IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/IEB/Business Studies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/IEB/Business Studies/$fileName';
