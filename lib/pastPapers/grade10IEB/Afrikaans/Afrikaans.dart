...
class AfrikaansGrade10IEBPage extends StatefulWidget {
  const AfrikaansGrade10IEBPage({super.key});

  @override
  _AfrikaansGrade10IEBPageState createState() =>
      _AfrikaansGrade10IEBPageState();
}

class _AfrikaansGrade10IEBPageState extends State<AfrikaansGrade10IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/IEB/Afrikaans');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/IEB/Afrikaans/$fileName';
