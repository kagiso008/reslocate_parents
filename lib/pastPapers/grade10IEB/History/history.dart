...
class HistoryGrade10IEBPage extends StatefulWidget {
  const HistoryGrade10IEBPage({super.key});

  @override
  _HistoryGrade10IEBPageState createState() => _HistoryGrade10IEBPageState();
}

class _HistoryGrade10IEBPageState extends State<HistoryGrade10IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/IEB/History');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/IEB/History/$fileName';
