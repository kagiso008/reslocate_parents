...
class DramaticArtsGrade10IEBPage extends StatefulWidget {
  const DramaticArtsGrade10IEBPage({super.key});

  @override
  _DramaticArtsGrade10IEBPageState createState() =>
      _DramaticArtsGrade10IEBPageState();
}

class _DramaticArtsGrade10IEBPageState
    extends State<DramaticArtsGrade10IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/IEB/Dramatic Arts');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/IEB/Dramatic Arts/$fileName';
