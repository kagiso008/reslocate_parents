...
class IsiZuluGrade10IEBPage extends StatefulWidget {
  const IsiZuluGrade10IEBPage({super.key});

  @override
  _IsiZuluGrade10IEBPageState createState() => _IsiZuluGrade10IEBPageState();
}

class _IsiZuluGrade10IEBPageState extends State<IsiZuluGrade10IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/IEB/IsiZulu');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/IEB/IsiZulu/$fileName';
