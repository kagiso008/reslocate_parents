...
class ConsumerStudiesGrade10IEBPage extends StatefulWidget {
  const ConsumerStudiesGrade10IEBPage({super.key});

  @override
  _ConsumerStudiesGrade10IEBPageState createState() =>
      _ConsumerStudiesGrade10IEBPageState();
}

class _ConsumerStudiesGrade10IEBPageState
    extends State<ConsumerStudiesGrade10IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/IEB/Consumer Studies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/IEB/Consumer Studies/$fileName';
