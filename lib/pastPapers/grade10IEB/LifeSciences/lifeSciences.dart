...
class LifeSciencesGrade10IEBPage extends StatefulWidget {
  const LifeSciencesGrade10IEBPage({super.key});

  @override
  _LifeSciencesGrade10IEBPageState createState() =>
      _LifeSciencesGrade10IEBPageState();
}

class _LifeSciencesGrade10IEBPageState
    extends State<LifeSciencesGrade10IEBPage> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/IEB/Life Sciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/IEB/Life Sciences/$fileName';
