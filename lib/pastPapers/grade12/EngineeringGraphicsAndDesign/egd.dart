...
class EGDGrade12Page extends StatefulWidget {
  const EGDGrade12Page({super.key});

  @override
  _EGDGrade12PageState createState() => _EGDGrade12PageState();
}

class _EGDGrade12PageState extends State<EGDGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/EngineeringGraphicsAndDesign');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/EngineeringGraphicsAndDesign/$fileName';
