...
class PhysicalSciencesGrade12Page extends StatefulWidget {
  const PhysicalSciencesGrade12Page({super.key});

  @override
  _PhysicalSciencesGrade12PageState createState() =>
      _PhysicalSciencesGrade12PageState();
}

class _PhysicalSciencesGrade12PageState
    extends State<PhysicalSciencesGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/PhysicalSciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/PhysicalSciences/$fileName';
