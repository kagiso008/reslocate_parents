...
class ConsumerStudiesGrade12Page extends StatefulWidget {
  const ConsumerStudiesGrade12Page({super.key});

  @override
  _ConsumerStudiesGrade12PageState createState() =>
      _ConsumerStudiesGrade12PageState();
}

class _ConsumerStudiesGrade12PageState
    extends State<ConsumerStudiesGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/ConsumerStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/ConsumerStudies/$fileName';
