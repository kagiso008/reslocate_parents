...
class SesothoGrade12Page extends StatefulWidget {
  const SesothoGrade12Page({super.key});

  @override
  _SesothoGrade12PageState createState() => _SesothoGrade12PageState();
}

class _SesothoGrade12PageState extends State<SesothoGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_12/Sesotho');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/Sesotho/$fileName';
