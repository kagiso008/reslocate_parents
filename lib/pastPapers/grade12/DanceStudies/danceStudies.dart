...
class DanceStudiesGrade12Page extends StatefulWidget {
  const DanceStudiesGrade12Page({super.key});

  @override
  _DanceStudiesGrade12PageState createState() =>
      _DanceStudiesGrade12PageState();
}

class _DanceStudiesGrade12PageState extends State<DanceStudiesGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/DanceStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/DanceStudies/$fileName';
