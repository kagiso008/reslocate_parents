...
class SASLGrade12Page extends StatefulWidget {
  const SASLGrade12Page({super.key});

  @override
  _SASLGrade12PageState createState() => _SASLGrade12PageState();
}

class _SASLGrade12PageState extends State<SASLGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/SouthAfricanSignLanguage');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/SouthAfricanSignLanguage/$fileName';
