...
class CATGrade12Page extends StatefulWidget {
  const CATGrade12Page({super.key});

  @override
  _CATGrade12PageState createState() => _CATGrade12PageState();
}

class _CATGrade12PageState extends State<CATGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/ComputerApplicationsTechnology');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/ComputerApplicationsTechnology/$fileName';
