...
class AfrikaansGrade12Page extends StatefulWidget {
  const AfrikaansGrade12Page({super.key});

  @override
  _AfrikaansGrade12PageState createState() => _AfrikaansGrade12PageState();
}

class _AfrikaansGrade12PageState extends State<AfrikaansGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_12/Afrikaans');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/Afrikaans/$fileName';
