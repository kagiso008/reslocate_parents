...
class MarineSciencesGrade12Page extends StatefulWidget {
  const MarineSciencesGrade12Page({super.key});

  @override
  _MarineSciencesGrade12PageState createState() =>
      _MarineSciencesGrade12PageState();
}

class _MarineSciencesGrade12PageState extends State<MarineSciencesGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/MarineSciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/MarineSciences/$fileName';
