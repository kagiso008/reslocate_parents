...
class HospitalityStudiesGrade12Page extends StatefulWidget {
  const HospitalityStudiesGrade12Page({super.key});

  @override
  _HospitalityStudiesGrade12PageState createState() =>
      _HospitalityStudiesGrade12PageState();
}

class _HospitalityStudiesGrade12PageState
    extends State<HospitalityStudiesGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/HospitalityStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/HospitalityStudies/$fileName';
