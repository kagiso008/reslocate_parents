...
class DramaticArtsGrade12Page extends StatefulWidget {
  const DramaticArtsGrade12Page({super.key});

  @override
  _DramaticArtsGrade12PageState createState() =>
      _DramaticArtsGrade12PageState();
}

class _DramaticArtsGrade12PageState extends State<DramaticArtsGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/DramaticArts');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/DramaticArts/$fileName';
