...
class IsiXhosaGrade12Page extends StatefulWidget {
  const IsiXhosaGrade12Page({super.key});

  @override
  _IsiXhosaGrade12PageState createState() => _IsiXhosaGrade12PageState();
}

class _IsiXhosaGrade12PageState extends State<IsiXhosaGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_12/IsiXhosa');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/IsiXhosa/$fileName';
