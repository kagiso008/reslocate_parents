...
class LifeSciencesGrade12Page extends StatefulWidget {
  const LifeSciencesGrade12Page({super.key});

  @override
  _LifeSciencesGrade12PageState createState() =>
      _LifeSciencesGrade12PageState();
}

class _LifeSciencesGrade12PageState extends State<LifeSciencesGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/LifeSciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/LifeSciences/$fileName';
