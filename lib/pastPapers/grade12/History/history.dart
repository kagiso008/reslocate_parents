...
class HistoryGrade12Page extends StatefulWidget {
  ...
  @override
  _HistoryGrade12PageState createState() => _HistoryGrade12PageState();
}
...
class _HistoryGrade12PageState extends State<HistoryGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_12/History');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/History/$fileName';
