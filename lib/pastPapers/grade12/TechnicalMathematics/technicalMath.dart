...
class TechnicalMathsGrade12Page extends StatefulWidget {
  const TechnicalMathsGrade12Page({super.key});

  @override
  _TechnicalMathsGrade12PageState createState() =>
      _TechnicalMathsGrade12PageState();
}

class _TechnicalMathsGrade12PageState extends State<TechnicalMathsGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/TechnicalMathematics');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/TechnicalMathematics/$fileName';
