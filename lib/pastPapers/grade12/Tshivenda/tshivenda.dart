...
class TshivendaGrade12Page extends StatefulWidget {
  const TshivendaGrade12Page({super.key});

  @override
  _TshivendaGrade12PageState createState() => _TshivendaGrade12PageState();
}

class _TshivendaGrade12PageState extends State<TshivendaGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_12/Tshivenda');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/Tshivenda/$fileName';
