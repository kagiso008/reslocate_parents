...
class VisualArtsGrade12Page extends StatefulWidget {
  const VisualArtsGrade12Page({super.key});

  @override
  _VisualArtsGrade12PageState createState() => _VisualArtsGrade12PageState();
}

class _VisualArtsGrade12PageState extends State<VisualArtsGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_12/VisualArts');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/VisualArts/$fileName';
