...
class ReligionStudiesGrade12Page extends StatefulWidget {
  const ReligionStudiesGrade12Page({super.key});

  @override
  _ReligionStudiesGrade12PageState createState() =>
      _ReligionStudiesGrade12PageState();
}

class _ReligionStudiesGrade12PageState
    extends State<ReligionStudiesGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_12/ReligionStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/ReligionStudies/$fileName';
