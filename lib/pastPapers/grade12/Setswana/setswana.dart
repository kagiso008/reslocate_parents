...
class SetswanaGrade12Page extends StatefulWidget {
  const SetswanaGrade12Page({super.key});

  @override
  _SetswanaGrade12PageState createState() => _SetswanaGrade12PageState();
}

class _SetswanaGrade12PageState extends State<SetswanaGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_12/Setswana');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/Setswana/$fileName';
