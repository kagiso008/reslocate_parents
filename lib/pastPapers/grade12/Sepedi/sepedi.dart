...
class SepediGrade12Page extends StatefulWidget {
  const SepediGrade12Page({super.key});

  @override
  _SepediGrade12PageState createState() => _SepediGrade12PageState();
}

class _SepediGrade12PageState extends State<SepediGrade12Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_12/Sepedi');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_12/Sepedi/$fileName';
